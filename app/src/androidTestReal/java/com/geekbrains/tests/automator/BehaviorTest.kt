...
class BehaviorTest {
    ...
    //Убеждаемся, что после нажатия searchButton при отсуствии сети totalCountTextView не отображается
    @Test
    fun test_TotalCountTextView_Invisible_AfterServerError() {

        // Включаем авиарежим, чтобы от сервера пришел ответ с ошибкой (Nexus_4_api_24)
        uiDevice.pressHome()
        uiDevice.swipe(500, 0, 500, 1500, 5)
        val airplaneModeButton: UiObject = uiDevice.findObject(UiSelector().description("Airplane mode"))
        if (!airplaneModeButton.isChecked) airplaneModeButton.click()
        ...
        // Выключаем airplaneMode:
        uiDevice.pressHome()
        uiDevice.swipe(500, 0, 500, 1500, 5)
        val airplaneButton: UiObject = uiDevice.findObject(UiSelector().description("Airplane mode"))
        if (airplaneModeButton.isChecked) airplaneButton.click()
    }
